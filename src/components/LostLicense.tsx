  ...
  const fetchRenewLicense = async () => {
    ...
    try {
      const { data } = await axiosInstance.post(
        `Applications/ApplyForReplacementLostLicenseApplicationAsync/${licenseId}`,
        null,
        {
          headers: {
  ...
  return (
    <div className="container mx-auto flex flex-col items-center justify-center gap-2 my-4">
      ...
      <div className="flex flex-col items-center justify-center gap-3 border overflow-hidden w-[400px] rounded hover:shadow-2xl dark:shadow-blue-400/40">
        <img
          src="https://www.godigit.com/content/dam/godigit/directportal/en/contenthm/lost-driving-licence.jpg"
          alt="lost license image"
        />
        <h2 className="font-bold text-lg px-2">Lost My License</h2>
        <p className="px-2">
          find your driving license online with ease. Follow the steps below to
