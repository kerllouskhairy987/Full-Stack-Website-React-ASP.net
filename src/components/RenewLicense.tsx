...
const RenewLicense = () => {
    ...
    const fetchRenewLicense = async () => {
        ...
        try {
            const { data } = await axiosInstance.post(`Applications/ApplyForRenewLicenseApplicantion/${licenseId}`, null, {
                headers: {
            ...
            AlertError({
                title: "Error",
                text: `${errorObj?.response?.data?.errors?.[0]}` || "Invalid license ID",
            })
        }
    }
        ...
        <div className="container mx-auto flex flex-col items-center justify-center gap-2 my-4">
            <div className="flex items-center justify-center my-10">
                <span className="animate-bounce relative inline-block before:absolute before:-inset-1 before:block before:-skew-y-3 before:bg-[#031F47]">
                    <h2 className="relative text-white font-semibold text-3xl p-2 mx-auto">Apply For New License</h2>
                </span>
            </div>
            <div className="flex flex-col items-center justify-center gap-3 border overflow-hidden w-[400px] rounded hover:shadow-2xl dark:shadow-blue-400/40">
                <img src="https://media.gettyimages.com/id/501924077/photo/smiling-teenage-girl-sitting-in-car-showing-driving-license-partial-view.jpg?s=612x612&w=gi&k=20&c=OspJNyoMuwH5AG2YHI1NjXcZbEmBF07IzgixEBJkFvU=" alt="renew license image" />
                <h2 className="font-bold text-lg px-2">Renew License</h2>
                <p className="px-2">Renew your driving license online with ease. Follow the steps below to complete your application.</p>
